...
import shared_data.communication.client_to_server.requests.ConfigureCallBackRequest;
...
public class ServerCommunication implements ClientToServerInterface
{
    ...
    private Socket callBackSocket = null;
    ...
    public void setCallBackSocket(Socket callBackSocket)
    {
        synchronized(callBackSocket)
        {
    ...
    public void configureCallBacks() throws IOException, ClassNotFoundException
    {
        
        String requestInJson = SendAndReceiveInformation.receiveDataTCP(socket);

        ConfigureCallBackRequest configureCallBackRequest = (ConfigureCallBackRequest)Serialization.deserializeObjectFromJson(requestInJson);

        this.setCallBackSocket(new Socket(socket.getInetAddress(), configureCallBackRequest.getPort()));
    ...
    public EnumActionResult uploadSong(User user, String songName, String author, int year, String album, float duration, String category, String fileName)
    {
        ...
        if(uploadSongResponse.getEnumActionResult() == EnumActionResult.UPLOAD_SONG_SUCCESSFULL)
        {
            try
            {
                ...
                ClientSendFileThread clientSendFileThread = new ClientSendFileThread(fileName, uploadFileSocket, this);
    ...
    public String downloadSong(String songName)
    {
        ...
        if(downloadSongResponse.getEnumActionResult() == EnumActionResult.DOWNLOAD_SONG_SUCCESSFULL)
        {
            try
            {
                Socket downloadSocket = new Socket(socket.getInetAddress(), downloadSongResponse.getPort());
                ClientReceiveFileThread clientReceiveFileThread = new ClientReceiveFileThread(downloadSocket, this, songName);
        ...
        return downloadSongResponse.getEnumActionResult().toString();
    }
    ...
    public ArrayList<String> downloadPlaylist(String playlistName)
    {
        ...
        return downloadPlaylistsResponse.getFilesNames();
    }
