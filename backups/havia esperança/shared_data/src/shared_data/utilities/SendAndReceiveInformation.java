...
public class SendAndReceiveInformation
{
     ...
     * @throws IOException
     */
    public static void sendDataMulticast(String objectInJson, MulticastSocket socket, InetAddress ip, int port, int numberOfServers) throws IOException
    {
        ...
        synchronized(socket)
        {
            while(sendedBytes < bytes.length)
            {
                ...
                try
                {
                    ...
                    {
                        responsePacket = new DatagramPacket(new byte[1], 1);
                        socket.receive(responsePacket);
                        if(responsePacket.getData()[0] == (byte)1)
                        {
                            allSuccessfull = true;
            ...
            while(true)
            {
                ...
                try
                {
                    for(numberOfAnswers = 0; numberOfAnswers < numberOfServers ; numberOfAnswers++) 
                    {
                        socket.receive(new DatagramPacket(new byte[0], 0));
                    }
                }
    ...
    public static String receiveDataMulticast(MulticastSocket socket)throws IOException, ClassNotFoundException, SocketTimeoutException
    {
        ArrayList<byte[]> arrayList = new ArrayList<byte[]>();
