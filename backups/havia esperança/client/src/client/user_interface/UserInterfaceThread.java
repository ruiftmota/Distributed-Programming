...
public class UserInterfaceThread extends Thread
{
    ...
    private void runDownloadSongPane()
    {
        ...
        EnumActionResult response = this.clientToServerInterface.downloadSong(parameters[0]);

        if(response == EnumActionResult.DOWNLOAD_SONG_FAILED)
        {
    ...
    private void runDownloadPlaylistPane()
    {
        ...
        EnumActionResult response = this.clientToServerInterface.downloadPlaylist(parameters[1]);
    ...
    private void runListDownloadedSongsPane()
    {
        ...
        {
            SynchronizedPrint.printLine(i + ". " + filesList[i]);
        }
    ...
    private void playSong(String fileName)
    {
        try
        {
            ...
            processBuilder.command("cmd.exe", "/c", "start lose_control.mp3");
